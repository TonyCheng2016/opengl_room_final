...
void CShape::SetShader(GLuint uiShaderHandle)
{
	...
	//---------------------light2----------------
	m_uiLightInView2 = glGetUniformLocation(m_uiProgram, "LightInView2");
...
//----------------light1 and light2--------------------------
vec4 CShape::PhongReflectionModelTwoLights(vec4 vPoint, vec3 vNormal, const LightSource &Lights1, const LightSource &Lights2)
{
	vec3 vN, vLDir;		// 用 vec3 來宣告是為了節省計算, 如果要讓程式寫起來更方便，可改用 vec4 來宣告
	vec4 vPosInView;
	vec4 vLightInView1, vLightInView2; //光源1和光源2
	vec4 vColorBoth, vColor1, vColor2;	// 該頂點的顏色
	...
	// 3. 計算 Ambient color :  Ia = Ka * Material.ambient * La
	vColor1 = m_Material.ka * m_Material.ambient * Lights1.diffuse;//light1
	vColor2 = m_Material.ka * m_Material.ambient * Lights2.diffuse;//light2

	// 4. 計算 Light 在鏡頭座標系統的位置
	//    此處假設 Light 的位置已經在世界座標的絕對位置
	vLightInView1 = m_mxView * Lights1.position;//light1
	vLightInView2 = m_mxView * Lights2.position;//light2

	vec3 vLight1; // 用 vec3 來宣告是為了節省計算, 如果要讓程式寫起來更方便，可改用 vec4 來宣告
	vLight1.x = vLightInView1.x - vPosInView.x;
	vLight1.y = vLightInView1.y - vPosInView.y;
	vLight1.z = vLightInView1.z - vPosInView.z;
	vLight1 = normalize(vLight1); // normalize light vector

	vec3 vLight2;// 用 vec3 來宣告是為了節省計算, 如果要讓程式寫起來更方便，可改用 vec4 來宣告
	vLight2.x = vLightInView2.x - vPosInView.x;
	vLight2.y = vLightInView2.y - vPosInView.y;
	vLight2.z = vLightInView2.z - vPosInView.z;
	vLight2 = normalize(vLight2); // normalize light vector

	vec3 vLightDir = m_mxMV3X3Final * Lights1.spotDirection;
	vec3 vLightDir2 = m_mxMV3X3Final * Lights2.spotDirection;

	// 5. 計算照明的點是否落在 spotCutoff 之內
	GLfloat fLdotLDir1 = -(vLight1.x*vLightDir.x + vLight1.y*vLightDir.y + vLight1.z*vLightDir.z);//light1
	GLfloat fLdotLDir2 = -(vLight2.x*vLightDir2.x + vLight2.y*vLightDir2.y + vLight2.z*vLightDir2.z);//light2

	//---------------light1--------------------

	if (fLdotLDir1 >= Lights1.spotCosCutoff)
	{
		// 5. 計算 L dot N
		GLfloat fLdotN1 = vLight1.x*vN.x + vLight1.y*vN.y + vLight1.z*vN.z;
		color4 fLightI1 = Lights1.diffuse * powf(fLdotLDir1, Lights1.spotExponent);

		// Diffuse Color : Id = Kd * Material.diffuse * Ld * (L dot N)
		vColor1 += m_Material.kd * m_Material.diffuse * fLightI1 * fLdotN1;

		// Specular color
		// Method 1: Phone Model
		//   計算 View Vector
		vec3 vView1;
		vView1.x = 0 - vPosInView.x;  // 目前已經以鏡頭座標為基礎, 所以 View 的位置就是 (0,0,0)
		vView1.y = 0 - vPosInView.y;
		vView1.z = 0 - vPosInView.z;
		vView1 = normalize(vView1);

		//	 計算 Light 的 反射角 vRefL
		vec3 vRefL1 = 2.0f * (fLdotN1)* vN - vLight1;

		vRefL1 = normalize(vRefL1);
		//   計算  vReflectedL dot View
		GLfloat RdotV = vRefL1.x*vView1.x + vRefL1.y*vView1.y + vRefL1.z*vView1.z;

		// Specular Color : Is = Ks * Ls * (R dot V)^Shininess;
		if (RdotV > 0) vColor1 += m_Material.ks * m_Material.specular * fLightI1 * powf(RdotV, m_Material.shininess);
	}

	//---------------------light2----------------

	if (fLdotLDir2 >= Lights2.spotCosCutoff)
	{
		// 5. 計算 L dot N
		GLfloat fLdotN2 = vLight2.x*vN.x + vLight2.y*vN.y + vLight2.z*vN.z;
		color4 fLightI2 = Lights2.diffuse * powf(fLdotLDir2, Lights2.spotExponent);

		// Diffuse Color : Id = Kd * Material.diffuse * Ld * (L dot N)
		vColor2 += m_Material.kd * m_Material.diffuse * fLightI2 * fLdotN2;

		// Specular color
		// Method 1: Phone Model
		//   計算 View Vector
		vec3 vView2;
		vView2.x = 0 - vPosInView.x;  // 目前已經以鏡頭座標為基礎, 所以 View 的位置就是 (0,0,0)
		vView2.y = 0 - vPosInView.y;
		vView2.z = 0 - vPosInView.z;
		vView2 = normalize(vView2);

		//	 計算 Light 的 反射角 vRefL
		vec3 vRefL2 = 2.0f * (fLdotN2)* vN - vLight2;

		vRefL2 = normalize(vRefL2);
		//   計算  vReflectedL dot View
		GLfloat RdotV2 = vRefL2.x*vView2.x + vRefL2.y*vView2.y + vRefL2.z*vView2.z;

		// Specular Color : Is = Ks * Ls * (R dot V)^Shininess;
		if (RdotV2 > 0) vColor2 += m_Material.ks * m_Material.specular * fLightI2 * powf(RdotV2, m_Material.shininess);
	}

	//---------------light1+light2的顏色-------------

	vColorBoth = vColor1 + vColor2;

	vColorBoth.w = 1; // Alpha 改設定成 1，預設都是不透明物體
	// Method 2: Modified Phone Model 
