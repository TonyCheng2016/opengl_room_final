...
void CShapeMT::SetShader(GLuint uiShaderHandle)
{
	// 改放置這裡, 方便每一個物件的設定
	CreateBufferObject();
...
void CShapeMT::SetViewMatrix(mat4 &mat)
{
	m_mxView = mat;
	m_bMVUpdated = true;
}
...
vec4 CShapeMT::PhongReflectionModel(vec4 vPoint, vec3 vNormal, const LightSource &lights)
{
