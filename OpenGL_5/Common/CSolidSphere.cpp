...
//-------------------------light1 and light2---------------
void CSolidSphere::RenderWithGouraudShading(const LightSource &light1, const LightSource &light2)
{
	// Method 1 : 對每一個 Vertex 都計算顏色
	for (int i = 0; i < m_iStacks; i++) {
		for (int j = 0; j < 2 * (m_iSlices + 1); j++) {
			m_pColors[i * 2 * (m_iSlices + 1) + j] = PhongReflectionModelTwoLights(m_pPoints[i * 2 * (m_iSlices + 1) + j], m_pNormals[i * 2 * (m_iSlices + 1) + j], light1, light2);
		}
	}
}
...
//--------------------light1和light2----------------
void CSolidSphere::UpdateTwoLights(float dt, const LightSource &light1, const LightSource &light2)
{
#ifdef LIGHTING_WITHCPU
	if (m_bViewUpdated || m_bTRSUpdated) { // Model View 的相關矩陣內容有更動
		m_mxMVFinal = m_mxView * m_mxTRS;
	...
	m_vLightInView = m_mxView * light1.position;// 將 light1 轉換到鏡頭座標再傳入
	m_vLightInView2 = m_mxView * light2.position;// 將 light2 轉換到鏡頭座標再傳入
	//// 算出 AmbientProduct DiffuseProduct 與 SpecularProduct 的內容
