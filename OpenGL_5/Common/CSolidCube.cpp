...
//-------------light1 and light2---------------
void CSolidCube::RenderWithGouraudShading(const LightSource &light1, const LightSource &light2)
{
	for (int i = 0; i < m_iNumVtx; i += 6) {
...
//--------------------light1和light2----------------
void CSolidCube::UpdateTwoLights(float dt, const LightSource &light1, const LightSource &light2)
{
#ifdef LIGHTING_WITHCPU
	if (m_bViewUpdated || m_bTRSUpdated) { // Model View 的相關矩陣內容有更動
		m_mxMVFinal = m_mxView * m_mxTRS;
